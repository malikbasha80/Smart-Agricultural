...
app.secret_key = os.environ.get("SECRET_KEY", "opticrop-default-key")
...
def predict():
    ...
    for idx in top_indices:
        ...
        ideal = CROP_IDEAL.get(crop_name.lower(), {})
        ...
        if match_status in ["excellent", "good"]:
            best_crop = crop_name
    ...
    history = session.get("history", [])
    history.insert(0, {"crop": best_crop, "confidence": round(top3[0]["confidence"], 1), "inputs": user_input})
